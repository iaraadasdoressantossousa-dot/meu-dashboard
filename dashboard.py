...
if uploaded_file is not None:
    ...
    if all(col in df.columns for col in colunas_obrigatorias):
        ...
        st.divider()
        st.bar_chart(data =pd.read_csv(uploaded_file), x=None, y= r['ROI'], x_label=None, y_label='ROI', color=r, horizontal=False, sort=True, stack=None, width="stretch", height="content", use_container_width=None)
    else:
